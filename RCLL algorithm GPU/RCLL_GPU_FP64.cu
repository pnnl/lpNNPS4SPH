...
#define cell_size 0.01 //// basically, this value is suggested consistent with R_CUT
#define NCX 100
#define NCY 100
#define NCZ 100
...
__global__ void search_neighbors(double *x, double *y, double *z, int *cell_indices, int *cell_offsets, int *cell_particle_list, double *relative_coordinates, int *neighbors, int *num_neighbors) {
...
__global__ void assign_particles_to_cells(double *x, double *y, double *z, int *cell_indices, int *cell_offsets, int *cell_particle_list, double *relative_coordinates)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= N) return;
    ...
    relative_coordinates[i*3] = x[i] - (cix+0.5f)*cell_size;
    relative_coordinates[i*3 + 1] = y[i] - (ciy+0.5f)*cell_size;
    relative_coordinates[i*3 + 2] = z[i] - (ciz+0.5f)*cell_size;
...
int main() {
    ...
    double *relative_coordinates = (double*)malloc(N * 3 * sizeof(double));
    ...
    cudaMalloc(&d_relative_coordinates, N * 3 * sizeof(double));
    ...
    search_neighbors<<<grid_dim, block_dim>>>(d_x, d_y, d_z, d_cell_indices, d_cell_offsets, d_cell_particle_list, d_relative_coordinates,d_neighbors, d_num_neighbors);
    ...
    cudaMemcpy(relative_coordinates, d_relative_coordinates, N * 3 * sizeof(double), cudaMemcpyDeviceToHost);
