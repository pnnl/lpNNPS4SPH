...
__global__ void search_neighbors(float *x, float *y, float *z, int *cell_indices, int *cell_offsets, int *cell_particle_list, float *relative_coordinates, int *neighbors, int *num_neighbors) {
...
__global__ void assign_particles_to_cells(float *x, float *y, float *z, int *cell_indices, int *cell_offsets, int *cell_particle_list, float *relative_coordinates)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= N) return;
...
int main() {
    ...
    float *relative_coordinates = (float*)malloc(N * 3 * sizeof(float));
    ...
    cudaMalloc(&d_relative_coordinates, N * 3 * sizeof(float));
    ...
    cudaMemcpy(relative_coordinates, d_relative_coordinates, N * 3 * sizeof(float), cudaMemcpyDeviceToHost);
