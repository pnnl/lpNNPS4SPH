...
__global__ void search_neighbors(__half *x, __half *y, __half *z, int *neighbors, int *num_neighbors, int start_j, int end_j) {
...
int main() {
    ...
    for (int i = 0; i < N; i++) {
        ...
        x[i] = __float2half(fx+0.0);
        y[i] = __float2half(fy+0.0);
        z[i] = __float2half(fz+0.0);
    }
    ...
    cudaMalloc((void **)&d_z, N*sizeof(half));
    cudaMalloc((void **)&d_neighbors, N*max_neighbor*sizeof(int));
    cudaMalloc((void **)&d_num_neighbors, N*sizeof(int));

    cudaMemcpy(d_x, x, N*sizeof(half), cudaMemcpyHostToDevice);
    cudaMemcpy(d_y, y, N*sizeof(half), cudaMemcpyHostToDevice);
    cudaMemcpy(d_z, z, N*sizeof(half), cudaMemcpyHostToDevice);
