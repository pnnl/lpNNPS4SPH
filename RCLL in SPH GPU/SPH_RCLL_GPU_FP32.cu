...
__global__ void calc_gradient(float *x, float *y,double *func, double *fx, double *ddx, int *neighbors, int *num_neighbors) {
...
__global__ void search_neighbors(int *cell_indices, int *cell_offsets, int *cell_particle_list, float *relative_coordinates, int *neighbors, int *num_neighbors) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= N) return;
    ...
    for (int iix_cell=ix_cellm; iix_cell<=ix_cellp; iix_cell++)
    {
        for (int iiy_cell=iy_cellm; iiy_cell<=iy_cellp;iiy_cell++)
        {
            for (int j = 0; j < cell_offsets[iix_cell*NCY+ iiy_cell]; j++)
            {
                ...
                if (ii != i)
                {
                    float dx = relative_coordinates[i*2] - relative_coordinates[ii*2] + ((ix_cell - iix_cell)*cell_size);
                    float dy = relative_coordinates[i*2+1] - relative_coordinates[ii*2+1] + ((iy_cell - iiy_cell)*cell_size);
                    float r = (dx*dx + dy*dy);
                    if (r <= R_CUT*R_CUT)
                    {
...
__global__ void assign_particles_to_cells(float *x, float *y,  int *cell_indices, int *cell_offsets, int *cell_particle_list, float *relative_coordinates)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= N) return;
...
int main() {
    ...
    float *relative_coordinates = (float*)malloc(N * 2 * sizeof(float));
    int *d_cell_indices, *d_cell_offsets, *d_cell_particle_list;
    float *d_relative_coordinates;
    ...
    cudaMalloc(&d_relative_coordinates, N * 2 * sizeof(float));
    ...
    cudaMemcpy(relative_coordinates, d_relative_coordinates, N * 2 * sizeof(float), cudaMemcpyDeviceToHost);
