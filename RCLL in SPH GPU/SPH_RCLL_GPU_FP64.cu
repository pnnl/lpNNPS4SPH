...
__global__ void calc_gradient(double *x, double *y,double *func, double *fx, double *ddx, int *neighbors, int *num_neighbors) {
    ...
    for (int j = 0; j < num_neighbors[i]; j++)
    {
        ...
        double df_x = func[ii] - func[i];
        double dx = x[i] - x[ii];
        double dy = y[i] - y[ii];
        double r = sqrt(dx*dx + dy*dy);
        double q = r/R_CUT*2.0;
        double factor = 60.0/(7.0*3.141593*R_CUT*R_CUT);
        double dwx = 0.0; 
        if (q >= 0.0 && q <= 1.0)
        {
...
__global__ void search_neighbors(int *cell_indices, int *cell_offsets, int *cell_particle_list, double *relative_coordinates, int *neighbors, int *num_neighbors) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= N) return;
    ...
    for (int iix_cell=ix_cellm; iix_cell<=ix_cellp; iix_cell++)
    {
        for (int iiy_cell=iy_cellm; iiy_cell<=iy_cellp;iiy_cell++)
        {
            for (int j = 0; j < cell_offsets[iix_cell*NCY+ iiy_cell]; j++)
            {
                ...
                if (ii != i)
                {
                    double dx = relative_coordinates[i*2] - relative_coordinates[ii*2] + ((ix_cell - iix_cell)*cell_size);
                    double dy = relative_coordinates[i*2+1] - relative_coordinates[ii*2+1] + ((iy_cell - iiy_cell)*cell_size);
                    double r = sqrt(dx*dx + dy*dy);
                    if (r <= R_CUT)
                    {
...
__global__ void assign_particles_to_cells(double *x, double *y,  int *cell_indices, int *cell_offsets, int *cell_particle_list, double *relative_coordinates)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= N) return;
    ...
    relative_coordinates[i*2] = x[i] - (cix+0.5f)*cell_size;
    relative_coordinates[i*2 + 1] = y[i] - (ciy+0.5f)*cell_size;
...
int main() {
    double x[N], y[N],func[N],fx[N],ddx[N];
    ...
    double *relative_coordinates = (double*)malloc(N * 2 * sizeof(double));
    int *d_cell_indices, *d_cell_offsets, *d_cell_particle_list;
    double *d_relative_coordinates;
    ...
    cudaMalloc(&d_relative_coordinates, N * 2 * sizeof(double));
    ...
    // Call the kernel
    assign_particles_to_cells<<<grid_dim, block_dim>>>(d_x, d_y, d_cell_indices, d_cell_offsets, d_cell_particle_list, d_relative_coordinates);
    ...
    cudaEventRecord(start);
    calc_gradient<<<grid_dim, block_dim>>>(d_x, d_y, d_func, d_fx, d_ddx, d_neighbors, d_num_neighbors);
    cudaEventRecord(stop);
    cudaEventSynchronize(stop);
    ...
    cudaMemcpy(relative_coordinates, d_relative_coordinates, N * 2 * sizeof(double), cudaMemcpyDeviceToHost);
    cudaMemcpy(fx, d_fx, N*sizeof(double), cudaMemcpyDeviceToHost);
    cudaMemcpy(ddx, d_ddx, N*sizeof(double), cudaMemcpyDeviceToHost);
